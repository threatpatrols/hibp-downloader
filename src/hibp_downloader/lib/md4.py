...
class MD4:
    ...
    def _process(self, chunks):
        for chunk in chunks:
            ...
            Xi = [3, 7, 11, 19]
            for n in range(16):
                var_i, j, k, l = map(lambda x: x % 4, range(-n, -n + 4))
                K, S = n, Xi[n % 4]
                hn = h[var_i] + MD4.F(h[j], h[k], h[l]) + X[K]
                h[var_i] = MD4.lrot(hn & MD4.mask, S)
            ...
            for n in range(16):
                ...
                hn = h[var_i] + MD4.G(h[j], h[k], h[l]) + X[K] + 0x5A827999
                h[var_i] = MD4.lrot(hn & MD4.mask, S)
            ...
            for n in range(16):
                var_i, j, k, l = map(lambda x: x % 4, range(-n, -n + 4))
                K, S = Ki[n], Xi[n % 4]
                hn = h[var_i] + MD4.H(h[j], h[k], h[l]) + X[K] + 0x6ED9EBA1
                h[var_i] = MD4.lrot(hn & MD4.mask, S)
